...
UNKNOWN_USERS_IMG_PATH = "/media/yagshy/'Local Disk'/share/desktop/projects/attendance-system-update"
...
JOBS = {
    ...
    "8":"8-nji synp okuwçy",    
    "9":"9-njy synp okuwçy",    
    "10":"10-nji synp okuwçy",    
    "11":"11-nji synp okuwçy",    
    "12":"Beyleki işgärler",   
}

def camera_changer(request):
    global which_camera
    which_camera = not which_camera
    print(which_camera, "Which Camera")
...
def recognizer(gray, x, y, w, h, img):
    ...
    if confidence < 75:
        ...
        day = date.day
        if which_camera:
            check_user = DetectedUsers.objects.filter(
                user_id = user_ID, 
                action=True, 
                created_at__year = year, created_at__month = month, created_at__day = day).first()
            print(check_user, "action True")
            if check_user is None:
                DetectedUsers.objects.create(user_id = user_ID, action = which_camera)
        else:
            check_user = DetectedUsers.objects.filter(
                user_id = user_ID, 
                action=False, 
                created_at__year = year, created_at__month = month, created_at__day = day).first()
            print(check_user, "action False")
            if check_user is None:
                DetectedUsers.objects.create(user_id = user_ID, action = which_camera)
    elif confidence > 100:
        global camRec
...
def detected_user(request):
    if request.method == "GET":
        users_array = []
        global which_camera
        detected_users_id = DetectedUsers.objects.filter(action = which_camera).order_by("-id")
...
def known_user_analysis(request, value):
    if request.method == "GET":
        data = []
        detected_users_id = DetectedUsers.objects.all().order_by("-id")
